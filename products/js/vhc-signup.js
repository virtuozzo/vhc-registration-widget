...
Vz.Widgets.VHCP = function (config) {
    var self = this;
    ...
    self.product = self.element.getAttribute('data-product') || false;
    self.widgetTitle = self.element.getAttribute('data-title') ? self.element.getAttribute('data-title') : "Virtuozzo Hybrid Cloud Sign-Up";
    self.sDefCountries = {
        "EU": "CH",
        "APAC": "SG",
        "NAMER": "US",
    ...
    self.pardotMapping = {
        'vhi' : 'https://go.virtuozzo.com/l/148051/2025-02-06/7pfpd1',
        'vap' : 'https://go.virtuozzo.com/l/148051/2025-02-06/7pfpjj',
        'db' : 'https://go.virtuozzo.com/l/148051/2025-02-06/7pfpjm',
        'kubernetes' : 'https://go.virtuozzo.com/l/148051/2025-02-06/7pfpjq',
    }
    self.signupErrors = {
    ...
    self.render = function (options) {
        ...
        sHtml = new EJS({url: self.baseUrl + 'vhc-products-signup/partial/widget'}).render({
            baseUrl: self.baseUrl,
            testing: self.testing,
            sProduct: self.product,
            sTitle: self.widgetTitle,
            oCountries: {
                ...
                if (response.result === 'error') {
                    ...
                    $.ajax({
                        ...
                        if (response.result === 'error') {
                            ...
                            if (self.pardotTracking) {
                                self.trackSalesForce(self.pardotMapping[self.product], {
    ...
    if (self.element) {
        ...
        if (['vhi', 'vap', 'db', 'kubernetes'].indexOf(self.product) !== -1) {
            self.render();
        } else {
            throw new Error('Use only provided products [vhi, vap, db, kubernetes]. Fill in data-product attr.');
        }
    } else {
...
jQuery(document).ready(function ($) {
    ...
    var $VHCP = $('.vhc-products-signup-widget'),
        $oVHCPwidgets = [];

    if ($VHCP.length > 0) {
        $.each($VHCP, function (index) {
            this.classList.add('loading');
            var oAtts = {
                oElement: this
            };
            $oVHCPwidgets[index] = new Vz.Widgets.VHCP(oAtts);
        });
    }
});